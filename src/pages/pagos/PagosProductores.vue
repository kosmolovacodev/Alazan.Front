    ...
    <div class="row items-center q-gutter-md q-mb-md">
      <div class="col">
        ...
        <div class="text-caption text-grey-6">
          {{ viendoHistorial ? 'Historial cerrado' : `Sede: ${authStore.nombreSedeActiva}` }}
        </div>
      </div>
    ...
    <q-dialog v-model="dialogCierre" persistent>
      <q-card style="min-width: 340px">
        <q-card-section class="bg-orange-8 text-white row items-center">
          ...
          <span class="text-h6">Corte del Día — Pagos</span>
        </q-card-section>
        <q-card-section class="q-pt-md">
          <p class="text-body2 text-grey-7 q-mb-md">
            Los pagos con estatus <strong>PAGADO</strong> pasarán al historial.
            Los pagos pendientes (<em>Solicitar</em>, <em>Pago Solicitado</em>, <em>Autorizado</em>) permanecen activos.
          ...
          <q-input
            ...
            @keyup.enter="ejecutarCierreProductores"
          >
            <template #append>
        ...
        <q-card-actions align="right" class="q-pa-md">
          ...
          <q-btn unelevated color="orange-8" label="Confirmar Corte"
                 :disable="nipCierre.length < 4" @click="ejecutarCierreProductores"/>
        </q-card-actions>
      </q-card>
    </q-dialog>
...
async function cargarDatos() {
  ...
  try {
    ...
    const [solicitudesRes, configRes, topeRes] = await Promise.all([
      api.get('/pagos/solicitudes',            { params: { sedeId, soloActivos: !viendoHistorial.value } }),
...
async function ejecutarCierreProductores() {
